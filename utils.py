...
def get_prompt(instruction: str, incontext: bool = True) -> str:
    ...
    if incontext:
        # return f"你是精通古今中文的智能翻譯助理，以下是用戶和助理之間的對話。你要對用戶的問題提供詳細並且精準的回答。問題是要翻譯文言文或白話文。USER是表示用戶的問題，ASSISTANT是助理的回答，這邊提供你兩個示範。你只應該回答翻譯後的文句，回答應該只有中文且注意順暢度，並且提供一種回答即可。\
        ...
        return f"你是精通古今中文的助理，以下是用戶和人工智能助理之間的對話。 \
             你要對用戶的問題提供有用、安全、詳細和禮貌的回答。將文言文翻譯成白話文，或白話文翻譯成文言文。\
             這邊提供你兩個範例。\
